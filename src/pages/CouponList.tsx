...
const CouponList = () => {
    ...
    const fetchCoupons = async (activeTab: string, page = 1) => {
        setLoading(true);
        try {
        ...
        const init = async () => {
            ...
            try {
                const [availRes, allRes] = await Promise.all([
                    couponService.getAvailable(1, 10),
                    couponService.getAll(1, 1),
                ]);
                const availPageData: any = availRes.data;
                const allPageData: any = allRes.data;
                setCoupons(availPageData?.content || []);
                setTotalPages(availPageData?.totalPages || 0);
                setCurrentPage(availPageData?.number || 1);
