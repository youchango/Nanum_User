...
import type { Product, Review, ProductOption } from '../types/product';
...
const ReviewTab: React.FC<ReviewTabProps> = ({ productId, onCountChange }) => {
...
const QnATab: React.FC<{ productId: string }> = ({ productId }) => {
    const navigate = useNavigate();
    const [inquiries, setInquiries] = useState<any[]>([]); // Inquiry 타입이 없으면 우선 any[] 사용
    const [page, setPage] = useState<number>(0);
    const [totalPages, setTotalPages] = useState<number>(0);
    const [totalElements, setTotalElements] = useState<number>(0);
    const [loading, setLoading] = useState<boolean>(false);
    ...
    const [formData, setFormData] = useState<{ title: string, content: string, isSecret: boolean }>({ title: '', content: '', isSecret: false });
...
const ProductDetail = () => {
    ...
    const [quantity, setQuantity] = useState<number>(1); // 옵션 없는 상품용
    const [selectedOption, setSelectedOption] = useState<any>(null); // 옵션 선택 중간 상태
    const [selectedItems, setSelectedItems] = useState<{ option: ProductOption, quantity: number }[]>([]); // 선택된 옵션 목록
    ...
    useEffect(() => {
        const checkWishlist = async () => {
            ...
            try {
                ...
                const listData = res.data as any; // ApiResponse.data
                const list = listData.content || listData || [];
                const found = list.some((item: any) => item.productId === product.productId);
                setIsWished(found);
            } catch (e) {
    ...
    const hasOptions = product?.optionYn === 'Y' && (product?.options?.length || 0) > 0;

    // 옵션 선택 완료 시 목록에 추가
    const addSelectedItem = (opt: ProductOption) => {
    ...
    const updateItemQuantity = (optionId: number, delta: number) => {
        setSelectedItems(prev => prev.map(si =>
    ...
    const getOptionLabel = (opt: ProductOption) =>
            ...
            <div className="max-w-[1290px] mx-auto px-6 py-10 md:py-20">
                <div className="flex flex-col md:flex-row gap-12 lg:gap-20">
                    ...
                    <div className="flex-1 flex flex-col gap-6">
                        ...
                        {/* 옵션 선택 (셀렉트박스 방식) */}
                        {hasOptions && product && product.options && product.options.length > 0 && (() => {
                        ...
                        <div className="fixed bottom-0 left-0 w-full bg-white border-t border-gray-100 p-4 flex gap-2 z-[100] md:relative md:bg-transparent md:border-none md:p-0 md:mt-4 md:z-auto">
                            ...
                            <button onClick={isInactive ? undefined : handleAddToCart}
                                disabled={isInactive}
                            ...
                            <button onClick={isInactive ? undefined : handleDirectOrder}
                                disabled={isInactive}
