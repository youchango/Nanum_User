        ...
        const fetchOrder = async () => {
            ...
            try {
                ...
                setOrder({
                    ...
                    status: data.status,
                    statusText: ORDER_STATUS_MAP[data.status] || data.status,
                    ...
                    paymentInfo: {
                        method: PAYMENT_METHOD_MAP[data.paymentMethod] || data.paymentMethod || '',
                            ...
                            {order.items.map((item, idx) => (
                                <div key={idx} className="p-5 md:p-8 flex items-center gap-6 group">
                                    ...
                                    <div className="text-right hidden sm:block">
                                        <p className="text-[16px] font-black italic">{(item.totalPrice || item.price * item.quantity).toLocaleString()}원</p>
                                    </div>
                                </div>
                            ))}
                        </div>
