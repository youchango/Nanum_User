...
import type { SearchDTO } from '../types/common';
...
const ProductList = () => {
    ...
    const [selectedCategoryId, setSelectedCategoryId] = useState<{ main: number | null, sub: number | null }>({ main: null, sub: null });
    ...
    const fetchProducts = useCallback(async (pageNum: number, reset: boolean = false) => {
        ...
        try {
            ...
            const params: SearchDTO = {
                page: (isFixedTab ? 1 : pageNum), // 백엔드는 1부터 시작함
    ...
    const getDiscountRate = (price: number, suggestedPrice: number | null) => {
        if (!suggestedPrice || suggestedPrice <= price) return null;
        return Math.floor(((suggestedPrice - price) / suggestedPrice) * 100);
    };

    const getMainImage = (item: ProductListItem | Product) => {
    ...
    const handleAddToCart = (e: React.MouseEvent, item: ProductListItem) => {
        e.stopPropagation();
