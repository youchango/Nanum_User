...
const Checkout = () => {
    ...
    // Tax bill state
    const [taxBillType, setTaxBillType] = useState('NONE'); // NONE, TAX_BILL, CASH_RECEIPT
    ...
    useEffect(() => {
        const loadPointsAndCoupons = async () => {
            try {
                ...
                if (pointRes.status === 'fulfilled') setPointBalance(pointRes.value.data?.data ?? 0);
                if (couponRes.status === 'fulfilled') setAvailableCoupons(couponRes.value.data?.data || []);
            } catch (err) {
    ...
    const [savedTaxInfoList, setSavedTaxInfoList] = useState([]);
    const [showTaxInfoModal, setShowTaxInfoModal] = useState(false);
    const [taxModalEditId, setTaxModalEditId] = useState(null); // null=목록, id=수정
    ...
    const handleTaxModalSave = async () => {
        ...
        try {
            if (taxModalEditId === 'new') {
                ...
                await taxBillService.updateInfo(taxModalEditId, taxModalForm);
            }
            const res = await taxBillService.getInfoList();
            setSavedTaxInfoList(res.data?.data || []);
            setTaxModalEditId(null);
        } catch (e) {
    ...
    const handleTaxModalDelete = async (id) => {
    ...
    const handlePayment = async () => {
        ...
        const data = {
            ...
            paymentMethod: PAYMENT_METHOD_REVERSE_MAP[paymentMethod],
