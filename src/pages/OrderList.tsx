...
const OrderList = () => {
    ...
    const fetchOrders = async (pageNum: number = 1, sd: string = startDate, ed: string = endDate) => {
        setLoading(true);
        try {
            const params: any = { page: pageNum, size: ITEMS_PER_PAGE };
            if (sd && ed) {
                params.startDate = sd;
                params.endDate = ed;
            }
            const res = await orderService.getOrders(params);
            const data: PageResponse<Order> = res.data;
    ...
    const [reviewTarget, setReviewTarget] = useState<OrderItem & { orderId: number | string } | null>(null);
    const [reviewForm, setReviewForm] = useState({ title: '', content: '', rating: 5 });
    const [reviewSubmitting, setReviewSubmitting] = useState(false);
    const [reviewHover, setReviewHover] = useState(0);

    const openReviewModal = (item: OrderItem, orderId: number | string) => {
    ...
    const handleReviewSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await productService.createReview(String(reviewTarget.productId), {
                ...reviewForm,
    ...
    const onAddToCart = (item: OrderItem) => {
    ...
    const onDirectPurchase = (item: OrderItem) => {
                                        ...
                                        {(order.items || []).map((item: OrderItem, idx: number) => (
                                            <div key={idx} className="p-4 md:p-6">
                                                <div className="flex items-start md:items-center gap-4 md:gap-8 mb-4 md:mb-0">
                                                    ...
                                                    <div className="hidden md:flex flex-col gap-2 w-40">
                                                        ...
                                                        {order.status === 'DELIVERED' && !item.reviewYn && (
                                                            <button onClick={() => openReviewModal(item, order.orderId || order.id)} className="w-full py-2.5 border border-[#968064] text-[#968064] text-[11px] font-bold hover:bg-orange-50 transition-all">후기 작성</button>
                                                        )}
                                                        {order.status === 'DELIVERED' && item.reviewYn && (
                                                ...
                                                <div className="flex gap-2 md:hidden">
                                                    ...
                                                    {order.status === 'DELIVERED' && !item.reviewYn && (
                                                        <button onClick={() => openReviewModal(item, order.orderId || order.id)} className="flex-1 py-3 border border-[#968064] text-[#968064] text-[11px] font-bold active:bg-orange-50 transition-all rounded-sm">후기</button>
                                                    )}
                                                    {order.status === 'DELIVERED' && item.reviewYn && (
